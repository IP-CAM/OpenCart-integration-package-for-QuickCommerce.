		...
		if (isset($this->session->data['shipping_address'])) {
			...
			foreach ($results as $result) {
				if ($this->config->get($result['code'] . '_status')) {
					...
					$model = $this->{'model_shipping_' . $result['code']};
					$model->setAddress($this->session->data['shipping_address'], 'receiver'); // Test
					$model->setItems($this->cart->getProducts());
					$request = $model->formatQuoteObj();
