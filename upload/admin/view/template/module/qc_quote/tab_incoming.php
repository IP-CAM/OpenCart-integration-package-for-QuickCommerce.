...
$('#button-filter-view').on('click', function() {
	...
	$.ajax({
		url: "index.php?route=module/<?php echo $_name; ?>/getcustomers&token=<?php echo $token; ?>&store_id=<?php echo $store['store_id']; ?>&filter_name=" + filter_name,
		type: 'get',
		dataType: 'html',
...
$(document).ready(function(){
	$.ajax({
		url: "index.php?route=module/<?php echo $_name; ?>/getcustomers&token=<?php echo $token; ?>&page=1&store_id=<?php echo $store['store_id']; ?>",
		type: 'get',
		dataType: 'html',
		success: function(data) {		
			$("#ordersWrapper<?php echo $store['store_id']; ?>").html(data);
...
function removeAll() {      
	...
	if (r==true) {
		$.ajax({
			url: 'index.php?route=module/<?php echo $_name; ?>/removeallcustomers&token=<?php echo $token; ?>&store_id=<?php echo $store['store_id']; ?>',
			type: 'post',
			data: {'remove': r},
...
function moveAll() {      
	...
	if (r==true) {
		$.ajax({
			url: 'index.php?route=module/<?php echo $_name; ?>/moveallcustomers&token=<?php echo $token; ?>&store_id=<?php echo $store['store_id']; ?>',
			type: 'post',
			data: {'move': r},
			success: function(response) {
				location.reload();
