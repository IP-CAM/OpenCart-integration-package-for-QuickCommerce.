              ...
              <div class="row">
                <div class="col-sm-2">
                  <ul class="nav nav-pills nav-stacked" id="address">
                    <li class="active"><a href="#tab-vendor" data-toggle="tab"><?php echo $tab_general; ?></a></li>
                    <?php $address_row = 1; ?>
                  ...
                  <div class="tab-content">
                    <div class="tab-pane active" id="tab-vendor">
                      <div class="form-group required">
                        ...
                        <div class="col-sm-10">
                          <input type="text" class="form-control" name="display_name" value="<?php echo $display_name; ?>" placeholder="<?php echo $entry_name; ?>" id="input-display-name" class="form-control" />
...
function country(element, index, zone_id) {
	$.ajax({
		url: 'index.php?route=vendor/vendor/country&token=<?php echo $token; ?>&country_id=' + element.value,
		dataType: 'json',
		beforeSend: function() {
...
//$('#history').load('index.php?route=vendor/vendor/history&token=<?php echo $token; ?>&vendor_id=<?php echo $vendor_id; ?>');

$('#button-history').on('click', function(e) {
  e.preventDefault();

	$.ajax({
		url: 'index.php?route=vendor/vendor/history&token=<?php echo $token; ?>&vendor_id=<?php echo $vendor_id; ?>',
		type: 'post',
		dataType: 'html',
...
$('#transaction').load('index.php?route=vendor/vendor/transaction&token=<?php echo $token; ?>&vendor_id=<?php echo $vendor_id; ?>');

$('#button-transaction').on('click', function(e) {
  e.preventDefault();

  $.ajax({
		url: 'index.php?route=vendor/vendor/transaction&token=<?php echo $token; ?>&vendor_id=<?php echo $vendor_id; ?>',
		type: 'post',
		dataType: 'html',
...
$('#reward').load('index.php?route=vendor/vendor/reward&token=<?php echo $token; ?>&vendor_id=<?php echo $vendor_id; ?>');

$('#button-reward').on('click', function(e) {
	e.preventDefault();

	$.ajax({
		url: 'index.php?route=vendor/vendor/reward&token=<?php echo $token; ?>&vendor_id=<?php echo $vendor_id; ?>',
		type: 'post',
		dataType: 'html',
...
$('#ip').load('index.php?route=vendor/vendor/ip&token=<?php echo $token; ?>&vendor_id=<?php echo $vendor_id; ?>');

$('body').delegate('.button-ban-add', 'click', function() {
	var element = this;

	$.ajax({
		url: 'index.php?route=vendor/vendor/addbanip&token=<?php echo $token; ?>',
		type: 'post',
		dataType: 'json',
...
$('body').delegate('.button-ban-remove', 'click', function() {
	...
	$.ajax({
		url: 'index.php?route=vendor/vendor/removebanip&token=<?php echo $token; ?>',
		type: 'post',
		dataType: 'json',
		data: 'ip=' + encodeURIComponent(this.value),
		beforeSend: function() {
...
<?php foreach ($addresses as $address) { ?>
$('#tab-vendor .form-group[data-sort]').detach().each(function() {
	if ($(this).attr('data-sort') >= 0 && $(this).attr('data-sort') <= $('#tab-vendor .form-group').length) {
		$('#tab-vendor .form-group').eq($(this).attr('data-sort')).before(this);
	}

	if ($(this).attr('data-sort') > $('#tab-vendor .form-group').length) {
		$('#tab-vendor .form-group:last').after(this);
	}

	if ($(this).attr('data-sort') < -$('#tab-vendor .form-group').length) {
		$('#tab-vendor .form-group:first').before(this);
	}
});
<?php } ?>
//--></script></div>
