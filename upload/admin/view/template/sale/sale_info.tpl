        ...
        <div class="tab-content">
          <div class="tab-pane active" id="tab-sale">
            <table class="table table-bsaleed">
              ...
              <?php if ($sale_status) { ?>
              <tr>
                <td><?php echo $text_sale_status; ?></td>
                <td id="sale-status"><?php echo $sale_status; ?></td>
              </tr>
              <?php } ?>
              <?php if ($comment) { ?>
              <tr>
          ...
          <div class="tab-pane" id="tab-payment">
            <table class="table table-bsaleed">
              ...
              <tr data-sort="<?php echo $custom_field['sort_sale'] + 1; ?>">
                <td><?php echo $custom_field['name']; ?>:</td>
                <td><?php echo $custom_field['value']; ?></td>
              </tr>
              <?php } ?>
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-10">
                    <select name="sale_status_id" id="input-sale-status" class="form-control">
                      <?php foreach ($sale_statuses as $sale_statuses) { ?>
                      <?php if ($sale_statuses['sale_status_id'] == $sale_status_id) { ?>
                      <option value="<?php echo $sale_statuses['sale_status_id']; ?>" selected="selected"><?php echo $sale_statuses['name']; ?></option>
                      <?php } else { ?>
                      <option value="<?php echo $sale_statuses['sale_status_id']; ?>"><?php echo $sale_statuses['name']; ?></option>
                      <?php } ?>
                      <?php } ?>
                    </select>
                  </div>
...
$(document).delegate('#button-invoice', 'click', function() {
	$.ajax({
		url: 'index.php?route=sale/sale/createinvoiceno&token=<?php echo $token; ?>&sale_id=<?php echo $sale_id; ?>',
		dataType: 'json',
		beforeSend: function() {
		...
		success: function(json) {
			...
			if (json['error']) {
				$('#tab-sale').prepend('<div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i> ' + json['error'] + '</div>');
			}
...
$(document).delegate('#button-reward-add', 'click', function() {
	$.ajax({
		url: 'index.php?route=sale/sale/addreward&token=<?php echo $token; ?>&sale_id=<?php echo $sale_id; ?>',
		type: 'post',
		dataType: 'json',
		beforeSend: function() {
...
$(document).delegate('#button-reward-remove', 'click', function() {
	$.ajax({
		url: 'index.php?route=sale/sale/removereward&token=<?php echo $token; ?>&sale_id=<?php echo $sale_id; ?>',
		type: 'post',
		dataType: 'json',
		beforeSend: function() {
...
$(document).delegate('#button-commission-add', 'click', function() {
	$.ajax({
		url: 'index.php?route=sale/sale/addcommission&token=<?php echo $token; ?>&sale_id=<?php echo $sale_id; ?>',
		type: 'post',
		dataType: 'json',
		beforeSend: function() {
...
$(document).delegate('#button-commission-remove', 'click', function() {
	$.ajax({
		url: 'index.php?route=sale/sale/removecommission&token=<?php echo $token; ?>&sale_id=<?php echo $sale_id; ?>',
		type: 'post',
		dataType: 'json',
		beforeSend: function() {
...
$('#history').load('index.php?route=sale/sale/history&token=<?php echo $token; ?>&sale_id=<?php echo $sale_id; ?>');
...
$('#button-history').on('click', function() {
  if(typeof verifyStatusChange == 'function'){
	...
	$.ajax({
		url: 'index.php?route=sale/sale/api&token=<?php echo $token; ?>&api=api/sale/history&sale_id=<?php echo $sale_id; ?>',
		type: 'post',
		dataType: 'json',
		data: 'sale_status_id=' + encodeURIComponent($('select[name=\'sale_status_id\']').val()) + '&notify=' + ($('input[name=\'notify\']').prop('checked') ? 1 : 0) + '&append=' + ($('input[name=\'append\']').prop('checked') ? 1 : 0) + '&comment=' + encodeURIComponent($('textarea[name=\'comment\']').val()),
		beforeSend: function() {
		...
		success: function(json) {
			...
			if (json['success']) {
				...
				$('#sale-status').html($('select[name=\'sale_status_id\'] option:selected').text());			
			}			
		},			
		error: function(xhr, ajaxOptions, thrownError) {
			alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText);
...
function changeStatus(){
  var status_id = $('select[name="sale_status_id"]').val();

  $('#openbay-info').remove();

  $.ajax({
    url: 'index.php?route=extension/openbay/getsaleinfo&token=<?php echo $token; ?>&sale_id=<?php echo $sale_id; ?>&status_id='+status_id,
    dataType: 'html',
...
function addSaleInfo(){
  ...
  $.ajax({
    url: 'index.php?route=extension/openbay/addsaleinfo&token=<?php echo $token; ?>&sale_id=<?php echo $sale_id; ?>&status_id='+status_id,
    type: 'post',
    dataType: 'html',
...
$('select[name="sale_status_id"]').change(function(){ changeStatus(); });
//--></script>
