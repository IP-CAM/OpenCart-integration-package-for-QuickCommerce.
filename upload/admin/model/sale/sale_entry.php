...
class ModelSaleSaleEntry extends Model {
	...
	public function updateTables() {
		$query = $this->db->query("SHOW COLUMNS FROM `" . DB_PREFIX . "oe_sale_product` LIKE 'sale_product_id'");
		if ($query->num_rows < 1) {
			$this->db->query("ALTER TABLE `" . DB_PREFIX . "oe_sale_product` ADD COLUMN `sale_product_id` int(11) NOT NULL");
		}
	...
	public function install() {
		$this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "oe_sale` (
			`oe_sale_id` int(11) NOT NULL auto_increment,
			`sale_id` int(11) NOT NULL UNIQUE,
			PRIMARY KEY (`oe_sale_id`)
		);");
		$this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "oe_sale_product` (
			`oe_sale_product_id` int(11) NOT NULL auto_increment,
			`oe_sale_id` int(11) NOT NULL,
			`sale_product_id` int(11) NOT NULL,
			`sale_id` int(11) NOT NULL,
			`product_id` int(11) NOT NULL,
			`notax` tinyint(1) NOT NULL,
			PRIMARY KEY (`oe_sale_product_id`)
		);");
	...
	public function uninstall() {
		$this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "oe_sale`");
		$this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "oe_sale_product`");
