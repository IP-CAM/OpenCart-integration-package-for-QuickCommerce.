...
class ControllerVendorVendor extends Controller {
	...
	public function approve() {
		...
		if ($vendors && $this->validateApprove()) {
			$this->model_vendor_vendor->approve($this->request->get['vendor_id']);
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			if (!$result['approved']) {
				$approve = $this->url->link('vendor/vendor/approve', 'token=' . $this->session->data['token'] . '&vendor_id=' . $result['vendor_id'] . $url, 'SSL');
			} else {
				$approve = '';
			}
			...
			if ($login_info && $login_info['total'] >= $this->config->get('config_login_attempts')) {
				$unlock = $this->url->link('vendor/vendor/unlock', 'token=' . $this->session->data['token'] . '&email=' . $result['email'] . $url, 'SSL');
			} else {
				$unlock = '';
			}
			...
			$data['vendors'][] = array(
				...
				'approve'        => $approve,
				'unlock'         => $unlock,
				'edit'           => $this->url->link('vendor/vendor/edit', 'token=' . $this->session->data['token'] . '&vendor_id=' . $result['vendor_id'] . $url, 'SSL')
			);
		}
		...
		$data['sort_name'] = $this->url->link('vendor/vendor', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
		$data['sort_email'] = $this->url->link('vendor/vendor', 'token=' . $this->session->data['token'] . '&sort=c.email' . $url, 'SSL');
		$data['sort_status'] = $this->url->link('vendor/vendor', 'token=' . $this->session->data['token'] . '&sort=c.status' . $url, 'SSL');
		$data['sort_ip'] = $this->url->link('vendor/vendor', 'token=' . $this->session->data['token'] . '&sort=c.ip' . $url, 'SSL');
		$data['sort_date_added'] = $this->url->link('vendor/vendor', 'token=' . $this->session->data['token'] . '&sort=c.date_added' . $url, 'SSL');
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['vendor_id'])) {
			...
			$data['action'] = $this->url->link('vendor/vendor/edit', 'token=' . $this->session->data['token'] . '&vendor_id=' . $this->request->get['vendor_id'] . $url, 'SSL');
		}
		...
		if (isset($this->request->post['address'])) {
			...
			$data['addresses'] = $this->model_vendor_vendor->getAddresses($this->request->get['vendor_id']);
		} else {
		...
		if (isset($this->request->post['address_id'])) {
			...
			$data['address_id'] = $vendor_info['address_id'];
		} else {
			$data['address_id'] = '';
		}
	...
	protected function validateForm() {
		...
		if ((utf8_strlen($this->request->post['display_name']) < 3) || (utf8_strlen($this->request->post['display_name']) > 64)) {
		...
		$custom_fields = array(); //$this->model_sale_custom_field->getCustomFields();

		foreach ($custom_fields as $custom_field) {
			if (($custom_field['location'] == 'account') && $custom_field['required'] && empty($this->request->post['custom_field'][$custom_field['custom_field_id']])) {
				$this->error['custom_field'][$custom_field['custom_field_id']] = sprintf($this->language->get('error_custom_field'), $custom_field['display_name']);
			}
		}
			...
			foreach ($this->request->post['address'] as $key => $value) {
				...
				foreach ($custom_fields as $custom_field) {
					if (($custom_field['location'] == 'address') && $custom_field['required'] && empty($value['custom_field'][$custom_field['custom_field_id']])) {
						$this->error['address'][$key]['custom_field'][$custom_field['custom_field_id']] = sprintf($this->language->get('error_custom_field'), $custom_field['display_name']);
					}
				}
			}
		}
	...
	public function history() {
		...
		$results = $this->model_vendor_vendor->getHistories($this->request->get['vendor_id'], ($page - 1) * 10, 10);
		...
		$pagination->page = $page;
		$pagination->limit = 10;
		$pagination->url = $this->url->link('vendor/vendor/history', 'token=' . $this->session->data['token'] . '&vendor_id=' . $this->request->get['vendor_id'] . '&page={page}', 'SSL');

		$data['pagination'] = $pagination->render();

		$data['results'] = sprintf($this->language->get('text_pagination'), ($history_total) ? (($page - 1) * 10) + 1 : 0, ((($page - 1) * 10) > ($history_total - 10)) ? $history_total : ((($page - 1) * 10) + 10), $history_total, ceil($history_total / 10));

		$this->response->setOutput($this->load->view('vendor/vendor_history.tpl', $data));
	}
	...
	public function transaction() {
		...
		$results = $this->model_vendor_vendor->getTransactions($this->request->get['vendor_id'], ($page - 1) * 10, 10);
		...
		$pagination->page = $page;
		$pagination->limit = 10;
		$pagination->url = $this->url->link('vendor/vendor/transaction', 'token=' . $this->session->data['token'] . '&vendor_id=' . $this->request->get['vendor_id'] . '&page={page}', 'SSL');

		$data['pagination'] = $pagination->render();

		$data['results'] = sprintf($this->language->get('text_pagination'), ($transaction_total) ? (($page - 1) * 10) + 1 : 0, ((($page - 1) * 10) > ($transaction_total - 10)) ? $transaction_total : ((($page - 1) * 10) + 10), $transaction_total, ceil($transaction_total / 10));

		$this->response->setOutput($this->load->view('vendor/vendor_transaction.tpl', $data));
	}
	...
	public function reward() {
		...
		$results = $this->model_vendor_vendor->getRewards($this->request->get['vendor_id'], ($page - 1) * 10, 10);
		...
		$pagination->page = $page;
		$pagination->limit = 10;
		$pagination->url = $this->url->link('vendor/vendor/reward', 'token=' . $this->session->data['token'] . '&vendor_id=' . $this->request->get['vendor_id'] . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('vendor/vendor_reward.tpl', $data));
	}
	...
	public function ip() {
		...
		$results = $this->model_vendor_vendor->getIps($this->request->get['vendor_id'], ($page - 1) * 10, 10);

		foreach ($results as $result) {
			$ban_ip_total = $this->model_vendor_vendor->getTotalBanIpsByIp($result['ip']);

			$data['ips'][] = array(
				'ip'         => $result['ip'],
				'total'      => $this->model_vendor_vendor->getTotalVendorsByIp($result['ip']),
				'date_added' => date('d/m/y', strtotime($result['date_added'])),
				'filter_ip'  => $this->url->link('vendor/vendor', 'token=' . $this->session->data['token'] . '&filter_ip=' . $result['ip'], 'SSL'),
				'ban_ip'     => $ban_ip_total
			);
		}
		...
		$pagination->page = $page;
		$pagination->limit = 10;
		$pagination->url = $this->url->link('vendor/vendor/ip', 'token=' . $this->session->data['token'] . '&vendor_id=' . $this->request->get['vendor_id'] . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('vendor/vendor_ip.tpl', $data));
	}
	...
	public function addBanIp() {
		...
		if (isset($this->request->post['ip'])) {
			if (!$this->user->hasPermission('modify', 'vendor/vendor')) {
				...
				$this->model_vendor_vendor->addBanIp($this->request->post['ip']);
	...
	public function removeBanIp() {
		...
		if (isset($this->request->post['ip'])) {
			if (!$this->user->hasPermission('modify', 'vendor/vendor')) {
				...
				$this->model_vendor_vendor->removeBanIp($this->request->post['ip']);
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_email'])) {
			...
			foreach ($results as $result) {
				$json[] = array(
					'vendor_id'       => $result['vendor_id'],
					'display_name'              => strip_tags(html_entity_decode($result['display_name'], ENT_QUOTES, 'UTF-8')),
					'firstname'         => $result['firstname'],
					'lastname'          => $result['lastname'],
					'email'             => $result['email'],
					...
					'address'           => $this->model_vendor_vendor->getAddresses($result['vendor_id'])
				);
			}
		}
		...
		foreach ($json as $key => $value) {
			$sort_order[$key] = $value['display_name'];
		}
	...
	public function address() {
		...
		if (!empty($this->request->get['address_id'])) {
			...
			$json = $this->model_vendor_vendor->getAddress($this->request->get['address_id']);
		}
